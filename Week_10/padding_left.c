...
void pad_left(struct character_node *characters, char pad_character) {
    ...
    struct character_node *new_char 
    = malloc(sizeof(struct character_node));
    new_char->data = pad_character;
    new_char->next = characters;
    
    
    struct character_node*cur_char = new_char;
    struct character_node*next_char = cur_char->next;
    int string_length = 0;
