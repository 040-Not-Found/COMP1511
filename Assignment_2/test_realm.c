...
void test_add_location(void) {
    ...
    int num_realm = add_location(test_realm, "some_locations");
    int correct_num_location = 1;
    Location cur_location = test_realm->castle;
    while (cur_location != NULL) {
        correct_num_location++;
        cur_location = cur_location->next;
    }
    
    if (num_realm != correct_num_location) {
        printf(DOES_NOT_FOLLOW_SPEC);
        return;
    }
